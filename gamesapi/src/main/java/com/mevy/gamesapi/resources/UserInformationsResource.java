...
public class UserInformationsResource {
    ...
    @PreAuthorize("hasRole('ROLE_ADMIN')")
    @GetMapping("/{id}")
    public ResponseEntity<UserInformations> findById(@PathVariable Long id) {
    ...
    @PreAuthorize("isAuthenticated()")
    @PutMapping("/current")
    public ResponseEntity<Void> updateCurrentUserInformations(@RequestBody UserInformationsUpdateDTO userInformationsUpdateDTO) {
